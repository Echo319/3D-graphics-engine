...
private:
	...
	mat4x4 Matrix_PointAt(vec3d& pos, vec3d& target, vec3d& up)
	{
		...
		//New Right is just the crossproduct of up and forward
...
public:
	bool OnUserCreate() override
	{
		...
		matProj = Matrix_MakeProjection(90.0f, (float)ScreenWidth() / (float)ScreenHeight(), 0.1f, 1000.0f);
		return true;
	}
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		mat4x4 matTrans;
		matTrans = Matrix_MakeTranslation(0.0f, 0.0f, 9.0f);
		...
		// Collections of triangles to raster
		std::vector<triangle> vecTrianglesToRaster;

		// Clear screen
		FillRect(0, 0, ScreenWidth(), ScreenHeight(), olc::Pixel(135, 206, 235));


		//vec3d light_direction = { 0.0f, 1.0f, -1.0f };
		/*vec3d light_direction = { 0, vTarget.y, -vTarget.z };
		...
		for (auto tri : meshCube.tris) {
			...
			{
				//Illumination
				vec3d light_direction = { 0, vCameraRay.y, -vCameraRay.z };
				light_direction = Vector_Normalise(light_direction);

				triTransformed.illumination = std::max(0.1f, Vector_DotProduct(light_direction, normal));

				//Convert World space --> ViewSpace
				triViewed.p[0] = Matrix_MultiplyVector(matView, triTransformed.p[0]);
				triViewed.p[1] = Matrix_MultiplyVector(matView, triTransformed.p[1]);
				triViewed.p[2] = Matrix_MultiplyVector(matView, triTransformed.p[2]);
				triViewed.illumination = triTransformed.illumination;
				...
				for (int n = 0; n < nClippedTriangles; n++) {
					...
					triProjected.illumination = clipped[n].illumination;
			...
			// Sort back to front - Painters algorithm draws the back triangles first. Slow correct impl would be a depth buffer. 
			// without a perpixel reference z-Buffer is quite hard
			...
			for (auto& triToRaster : vecTrianglesToRaster)
			{
				...
				{
					FillTriangle(t.p[0].x, t.p[0].y, t.p[1].x, t.p[1].y, t.p[2].x, t.p[2].y, olc::Pixel(255 * t.illumination, 255 * t.illumination, 255 * t.illumination));
